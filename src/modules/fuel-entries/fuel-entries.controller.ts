...
export class FuelEntriesController {
  ...
  async getSummary(@Query() query: any) { return this.service.getSummary(query); }

  @Get(':entry_id')
  @ApiOperation({ summary: 'Get fuel entry' })
  async findOne(@Param('entry_id', ParseIntPipe) id: number) { return this.service.findOne(id); }

  @Put(':entry_id')
  @ApiOperation({ summary: 'Update fuel entry' })
  async update(@Param('entry_id', ParseIntPipe) id: number, @Body() dto: any) { return this.service.update(id, dto); }

  @Delete(':entry_id')
  @ApiOperation({ summary: 'Delete fuel entry' })
  async remove(@Param('entry_id', ParseIntPipe) id: number) { return this.service.remove(id); }
}
