...
import { CreateUserDto, UpdateUserDto, AdminUserCreateDto, AdminUserUpdateDto } from './dto/user.dto';
...
export class UsersService {
  ...
  async findAll(skip = 0, limit = 100) {
    return this.db.select().from(schema.users).offset(skip).limit(limit).execute(); // use execute() explicitly if needed, or just await
  }
  ...
  async create(createUserDto: CreateUserDto) {
    ...
    const [id] = await this.db.insert(schema.users).values({
  ...
  async update(id: number, updateUserDto: UpdateUserDto) {
    ...
    if (updateUserDto.password) values.password = await bcrypt.hash(updateUserDto.password, 10);
    if (updateUserDto.full_name) values.full_name = updateUserDto.full_name;
    if (updateUserDto.is_active !== undefined) values.is_active = updateUserDto.is_active;
    if (updateUserDto.is_superuser !== undefined) values.is_admin = updateUserDto.is_superuser;

    await this.db.update(schema.users).set(values).where(eq(schema.users.id, id));
  ...
  async updateAdmin(id: number, updateUserDto: AdminUserUpdateDto) {
    ...
    if (role_ids !== undefined) {
      await this.db.delete(schema.users_to_roles).where(eq(schema.users_to_roles.user_id, id));
  ...
  async getUserPermissions(userId: number) {
    ...
    }).from(schema.users_to_roles).where(eq(schema.users_to_roles.user_id, userId));
    ...
    if ((user as any).is_admin) {
      ...
      for (const r of roles) {
        const rp = await this.db.select({
          name: schema.permissions.name
        }).from(schema.roles_to_permissions)
          .innerJoin(schema.permissions, eq(schema.permissions.id, schema.roles_to_permissions.permission_id))
  ...
  async getUserRoles(userId: number) {
    const roles = await this.db.select({
      ...
      .innerJoin(schema.roles, eq(schema.roles.id, schema.users_to_roles.role_id))
