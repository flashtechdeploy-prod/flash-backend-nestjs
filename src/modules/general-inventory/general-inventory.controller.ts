...
export class GeneralInventoryController {
  ...
  @Put('items/:item_code')
  async updateItem(@Param('item_code') itemCode: string, @Body() dto: any) {
  ...
  async issue(@Param('item_code') itemCode: string, @Body() dto: any) {
    return this.service.issue(itemCode, dto);
  }

  @Post('items/:item_code/return')
  async returnItem(@Param('item_code') itemCode: string, @Body() dto: any) {
