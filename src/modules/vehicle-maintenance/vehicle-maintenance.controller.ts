...
export class VehicleMaintenanceController {
  ...
  async findOne(@Param('record_id', ParseIntPipe) id: number) { return this.service.findOne(id); }

  @Put(':record_id')
  @ApiOperation({ summary: 'Update maintenance record' })
  async update(@Param('record_id', ParseIntPipe) id: number, @Body() dto: any) { return this.service.update(id, dto); }

  @Delete(':record_id')
  @ApiOperation({ summary: 'Delete maintenance record' })
  async remove(@Param('record_id', ParseIntPipe) id: number) { return this.service.remove(id); }
}
