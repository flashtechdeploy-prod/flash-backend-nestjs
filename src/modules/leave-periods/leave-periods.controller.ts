...
export class LeavePeriodsController {
  ...
  async getAlerts(@Query() query: any) { return this.service.getAlerts(query); }

  @Put(':leave_period_id')
  @ApiOperation({ summary: 'Update leave period' })
  async update(@Param('leave_period_id', ParseIntPipe) id: number, @Body() dto: any) { return this.service.update(id, dto); }

  @Delete(':leave_period_id')
  @ApiOperation({ summary: 'Delete leave period' })
  async remove(@Param('leave_period_id', ParseIntPipe) id: number) { return this.service.remove(id); }
}
